...
% Fryba\citet{fryba1996dynamics} described the first two sources in mathematical terms:
...
% \citet[Table 9.1]{fryba1996dynamics} gives sample of natural frequencies of steel truss bridges with open deck. Shown as follow Table~\ref{tab:spatialvibrationsteel}. Please note that truss bridges have high stiffness.
...
% 	\caption{Example of a track lateral alignment irregularities profile for a track with low irregularities in a total length of 1209m.  Extracted from \citet{da2007dynamic}}
% 	\label{fig:trackirrg}
% \end{figure}


% \subsubsection{Centrifugal forces}
% In \citet[6.5.1]{EC12} specifies following principles about centrifugal forces act on railway bridges:
...
% The centrifugal forces should be taken to act outwards in a horizontal direction at a height of 1.80m above the running surface (see \citet[Figure 1.1]{EC12}). For some traffic types, e.g. double stacked containers, an increased value of $h_t$ should be specified.
...
% $Q_{vk}$,$q_{vk}$ & Characteristic values of the vertical loads specified in \citet[6.3]{EC12}\\
% $f$ & Reduction factor, see below \\
% $v$ & Maximum speed in accordance with \citet[6.5.1(5)]{EC12}[m/s]\\
% $V$ & Maximum speed in accordance with \citet[6.5.1(5)]{EC12}[km/h]\\
...
% According to \citet[9.1.3]{fryba1996dynamics}, the horizontal lateral forces of railway vehicles act on the rial top level,i.e. outside the cross section centroid of the bridge in the majority of cases. Let the difference of elevations be $ h $. Consequently, they affect the bridge by twisting moments $ hH_n(t) $. The differential equation of a beam due to simple torsion is 
% \begin{equation}
...
% According to \citet[Chapter.2]{fryba1996dynamics}, theoretical models of railway bridges are of two types
...
% If the inertia effects are neglected, loads of the moving trains can be modelled as moving vertical forces. For example, load diagram for type TALGO trains is shown in Figure.\ref{fig:verticalmodel} proposed by \citet{uic}.
...
% 	\caption{A dynamic model for the vertical interaction of the rail track and wagon system. Proposed in \citet{sun2002dynamic}}
...
% \section{Track model}
% Proposed in \citet[A.6.1.3]{uic}, the track is represented by Timoshenko beam elements for the rails and takes account of the rail/sleeper fastening characteristics as well as the ballast(if one exists).
...
% 	\caption{Diagram of the dynamic train-track-bridge model. Extracted from \citet[Fig. 15]{uic}}
...
% \citet{EC0} propose following requirements
...
% For example, in  \citet[cl.8.7.4]{calgaro2010designers}, it is stated that if the bridge meets criteria ~\ref{cr:nocheck} , no dynamic analysis in necessary. 
...
% This logic diagram is used to determine whether a dynamic analysis is required, as shown in Figure~\ref{fig:logicdiagram}, is represented in \citet{EC12} Chapter 6.4 Dynamic Effects, where $V [Km/h]$ is the Maximum Line Speed at the site, $L [m]$, the span length, $n_0 [Hz]$, the first nature frequency under permanent loads, $n_T$, the first natural torsional frequency for the same load, $v [m/s]$ the maximum nominal speed and finally $(v/n_0)_{lim}$, as given in Annex F of EN1991-2. The frequency first of vibration, $n_0$, must be within the limits established in figure~\ref{fig:frequencylimit}.
...
%     	\caption{Flow chart for determining whether a dynamic analysis is required. Extracted from EN1991-2\citet{EC12}}
%     	\label{fig:logicdiagram}
% 	\end{subfigure}
% 	\begin{subfigure}[b]{0.45\textwidth}
%     	\includegraphics[width=\textwidth]{frequencylimit.pdf}
%     	\caption{Limits for bridge natural frequencies, $n_0 [Hz]$, as a function $L [m]$. Extracted from EN1991-2\citet{EC12}.}
%     	\label{fig:frequencylimit}
% 	\end{subfigure}
% \caption{Logic diagram for determining whether dynamic analyses are necessary, extracted from \citet[6.4.4]{EC12}}
% \label{logicandlimit}
% \end{figure}

% \subsection{Train models}\label{sec:train-models}
% According to NEN 1991-2\citet{EC12} and Designers Guide\citet{calgaro2010designers} Rail traffic actions are defined by means of load models, Four models of railway loading are given:
...
% 	\caption{Load Model 71 and characteristic values for vertical loads. Extracted from EN1991-2\citet{EC12}}
% 	\label{lm71}
% \end{figure}
...
% 	\caption{Load Models SW/0 and SW/2.. Extracted from EN1991-2\citet{EC12}}
% 	\label{lmsw}
% \end{figure}
...
% 	\caption{Diagram of Universal Dynamic Train HSLM-A. Extracted from EN 1991-2\citet{EC12}}
% 	\label{fig:hslma}
% \end{figure}
...
% 		\hline		
% 	\end{tabular}
% 	\caption{HSLM-A, definition of ten trains. Extracted from EN1991-2\citet{EC12}}
% 	\label{tab:hslma}
% \end{table}
...
% 	\caption{Diagram of Universal Dynamic Train HSLM-B. Extracted from EN 1991-2\citet{EC12}}
...
% 	\caption{Universal Dynamic Train HSLM-B. Extracted from EN 1991-2\citet{EC12}}
...
% 	\end{tabular}
% 	\caption{Application of HSLM-A and HSLM-B. Data extracted from EN 1991-2\citet{EC12}}
...
% 		\end{equation}\\
% 	where $ L_\varPhi $ is the `determinant' length (length associated with $ \varPhi $ ) in metres as defined in Table 6.2, EN1991-2\citet{EC12}.
% \end{enumerate}
...
% In designers' guide\citet{calgaro2010designers}, bridge parameters on dynamic effects are discussed.
...
% EN 1991-2\citet{EC12} doesn't specify the method of dynamic analysing, but UIC Leaflet 776\textsuperscript{2}\citet{uic} indicates that  for normal bridges there are 4 methods of analysing(an approximate method and three simplified method). However, 776\textsuperscript{2}\citet{uic} also indicates for special structures (bridges with long spans such as bowstring bridges), have to be solved using generic finite element (FEM) programs.

% Analysing using FEM programs can be very time and money consuming providing a complicated structure. Fortunately UIC Leaflet 776\textsuperscript{2}\citet{uic} is an optional reference so FEM analysing is not required in EN 1991-2 \citet{EC12}. Thus finding a simplified way of analysing complicated bridge structures is vital. The development of dynamic analysing methods will be discussed in Chapter.\ref{sec:dynamic-analysing-methods}
...
% 	\caption{Definition of deck twist. Extracted from \citet[Figure A2.1]{1990a2}}
...
% 	\caption{Definition of anular rotations at the end of the decks. Extracted from \citet[Figure A2.2]{1990a2}}
...
% For these type of verifications \citet{1990a2} defines the limiting values for the maximum vertical deflection for passenger comfort, as following:
...
% 		\hline
% 	\end{tabular}
% 	\caption{Recommended accelerations values to ensure the respective levels of comfort. Extracted from \citet[Table A 2.9]{1990a2}.}
...
% 	\caption{Maximum permissible vertical deflection $\delta$ for railway bridges with 3 or more successive simply supported spans corresponding to a permissible vertical acceleration of $b_v=1m/s^2$ in a coach for speed $V$[km/h]. Extracted from \citet[Figure A2.3]{1990a2}}
...
% LIR method is based on residual influence line. It is applicable only on statistically determined bridges, too. The solution of the displacements and accelerations in the midspan, of the simply supported beam, is developed by \citetauthor{dominguez2001dinamica}. The solution is given as follows:
...
% 	\caption{Train dynamic signatures for the seven real trains, considering a damping value of $\xi = 0.00$. Extracted from \citet[Figure 2.20]{da2007dynamic}}
% 	\label{trainsig}
% \end{figure}
...
% 	\caption{Nodal force time history definition for a single moving load $F$, with speed $v$. Extracted from \citet[Figure 2.15]{da2007dynamic}}
...
% 	\caption{Model for analysis of vehicle-structure system. Extracted from \citet[Figure 12]{lei2002analyses}}
...
% 	\caption{Load train with vehicle-bridge interaction: simplified interaction model and variables definition. Extracted from \citet[Figure15]{lei2002analyses}}
